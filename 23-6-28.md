...
先上官网下载linux的es包[网址](https://www.elastic.co/cn/downloads/elasticsearch)，然后上传解压，进入config目录编辑配置

先编辑jvm.options,编写jvm的内存大小，写两行Xxm内存大小g/m都行

注意es是不能安装在root下的。这里推荐用官方的docker安装。

继续配置，编辑elasticsearch.yml设置下data和log的目录，然后设置netwoork.host，设置访问ip即可。

然后要切换到root下 `vim /etc/security/limits.conf`在文件的结尾写上
...
然后执行 `sysctl -w vm.max_map_count=262144`

切换用户执行es解压目录下的bin目录下的elasticsearch就成功启动。
...
下面简单介绍如何使用es的api，我学习api是基于这篇[文章](https://blog.csdn.net/weixin_44396392/article/details/125377306)

es在导入数据之前也要进行类似建表的操作。这里叫做创建映射。而且其中更改表的操作用put，插入数据用post

一般使用postman也可以，也可以用kibana来，kibana比较方便。
...
在上面的案例中，不仅会查询到小米手机，而且与小米相关的都会查询到，多个词之间是or的关系。（小米手机分为小米和手机两个词，由于是or关系，所以只要有小米或者手机两个关键字中的一个就会被查询到）
...
这样_source字段中只有title和price两个字段了
```
...
[简书链接](https://www.jianshu.com/p/28b803a88d40)
